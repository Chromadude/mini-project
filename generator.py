...
def game(square_size):
    ...
    while not sum(playable):
        ...
        types_of_piece = get_types_of_piece(min(pieces_per_player, min(7, min(board_size)//2)))
        legal_moves = get_moves(types_of_piece, (min(board_size) + 1) // 2)
...
def get_moves(num_of_pieces, max_move_length):
    ...
    for i in range(num_of_pieces):
        ...
        while not_right:
            not_right = 0
            directional_move = [random.randrange(-1, 4) for _ in range(max_move_length)]
            raw_move = []
            for j in range(4):
                x, y = 0, 0
                ...
                if j == 0:
                    for move in all_moves:
                        for option in move:
                            if option[0] == x and option[1] == y:
                                not_right = True

                if (x != 0 or y != 0) and not not_right:
                    raw_move.append([x, y])
                else:
                    not_right = 1
            if not not_right:
